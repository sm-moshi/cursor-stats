...
import type { ProgressBarSettings } from "../interfaces/types";
import { t } from "./i18n";
 ...
 */
export function calculateDailyRemaining(currentRequests: number, limitRequests: number, periodEndDate: Date): string {
	const config = vscode.workspace.getConfiguration("cursorStats");
	const excludeWeekends = config.get<boolean>("excludeWeekends", false);
	...
	if (excludeWeekends) {
		...
		const now = new Date();
		const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
		remainingDays = Math.max(0, Math.ceil((periodEndDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)));
	}
